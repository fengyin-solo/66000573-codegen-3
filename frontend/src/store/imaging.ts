...
import type { VolumeData, ROIResult, WindowPreset, CustomWindowPreset, PresetValidation } from '@/types'
...
/** 后端不可用时的兜底固定方案 */
export const FALLBACK_FIXED_PRESETS: Record<string, WindowPreset> = {
...
/** 首次使用 / 恢复默认时填充的用户方案 */
const BUILTIN_CUSTOM_PRESETS: Omit<CustomWindowPreset, 'id'>[] = [
...
function fixedShortName(desc: string): string {
...
export const useImagingStore = defineStore('imaging', () => {
  ...
  const activePresetId = ref<string>(DEFAULT_PRESET_ID)

  function loadCustomPresets(): CustomWindowPreset[] {
    try {
  ...
  const customPresets = ref<CustomWindowPreset[]>(loadCustomPresets())
  ...
  function fixedPresets(): Record<string, WindowPreset> {
    return volumeData.value?.windowPresets || FALLBACK_FIXED_PRESETS
  }

  /** 按 id 找到固定方案，找不到返回 undefined */
  function findFixed(id: string): WindowPreset | undefined {
    if (!id.startsWith(FIXED_PREFIX)) return undefined
  ...
  function validatePreset(
    win: number, lev: number, name: string, excludeId?: string,
  ): PresetValidation[] {
    const errors: PresetValidation[] = []
    ...
    if (!trimmed) {
      ...
      const dupCustom = customPresets.value.some(p =>
        p.id !== excludeId && p.name.trim().toLowerCase() === lower)
      const fixed = fixedPresets()
      const dupFixed = Object.entries(fixed).some(([key, p]) =>
        key.toLowerCase() === lower || fixedShortName(p.desc).toLowerCase() === lower)
  ...
  function saveCurrentPreset(name: string): PresetValidation[] {
    const win = windowVal.value, lev = levelVal.value
    const errors = validatePreset(win, lev, name)
    if (errors.length) return errors
    const cp: CustomWindowPreset = { id: genId(), name: name.trim(), window: win, level: lev }
  ...
  function renameCustomPreset(id: string, newName: string): PresetValidation[] {
    const target = customPresets.value.find(p => p.id === id)
    if (!target) return [{ field: 'name', message: '方案不存在或已被删除' }]
  ...
  function deleteCustomPreset(id: string) {
    ...
    if (i >= 0) customPresets.value.splice(i, 1)
  ...
  async function loadVolume() {
    ...
    try {
      ...
      if (activePresetId.value) applyPreset(activePresetId.value)
